...
def test_create_binary_image():
    ...
    restore_image = Image.open(io.BytesIO(event.data))
    assert restore_image.size == image_expected_shape

    reconstruct_event = from_http(body, headers, data_unmarshaller=lambda x: x)

    # # Test cloudevent extension from http fields and data
    assert isinstance(body, bytes)
    assert body == image_bytes
    assert reconstruct_event.data == body
...
def test_create_structured_image():
    ...
    reconstruct_event = from_http(body, headers, data_unmarshaller=lambda x: x)
    assert reconstruct_event.data == image_bytes
